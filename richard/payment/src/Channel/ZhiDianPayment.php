...
class ZhiDianPayment  extends PaymentInterface
{

    private $pre_url = "https://api.azlab.cc/";
    ...
    {
        parent::__construct($filename ?: 'ZhiDianPayment');
        $this->pre_name = "支点支付，";
        $this->channel_id = App::make(ChannelIdByClassNameService::class)->excute("ZhiDianPayment");
    ...
    public function deposit($deposit_order_id = 0)
    {
        ...
        $data = [
            ...
            'gateway' => optional(collect(config('default.payment'))->where('id',$this->deposit_order->payment_id)->first())['code'],
            'ip' => "47.57.70.202",
            'currency' => 'CNY',
    ...
    public function transfer($transfer_order_id = 0)
    {
        ...
        try {
            ...
            $this->getChannelAccount($this->transfer_order->amount,2,5);
        }catch (\Exception){
            return $this->data;
        }
        $data = [
