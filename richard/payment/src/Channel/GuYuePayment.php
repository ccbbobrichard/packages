...
class GuYuePayment extends PaymentInterface
{
    protected $mid;
    protected $api_key;

    private $pre_url = "http://k6fhmfxxmxi.itxtpy.com/";

    function __construct($filename = "")
    {
        parent::__construct($filename ?: 'GuYuePayment');
        $this->pre_name = "古悦支付，";
        $this->channel_id = App::make(ChannelIdByClassNameService::class)->excute("GuYuePayment");
    ...
    public function parseChannel(){
        ...
        if($this->deposit_order->payment_id == 24){ //微信京东话费
            return 201;
        }
        if($this->deposit_order->payment_id == 25){ //沃尔玛点卡
            return 204;
        }
        return;
    }
